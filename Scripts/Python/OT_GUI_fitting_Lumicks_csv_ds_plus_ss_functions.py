...
def getCSV ():
    ...
    #print (df)
    PD=df.iloc[:,0] # to select only certain part of the imported table [Row,column]
    F=df.iloc[:,1]
    PD_nm=PD*1000
...
def fitting_ds(PD_ds, F_ds, Lc_ds, Lc_range):
    ...
    fit_ds["DNA/f_offset"].upper_bound=2
    ...
    fit_ds["DNA/St"].lower_bound = 400
    fit_ds["DNA/St"].upper_bound = 2000
...
def Fitting_WLC_ds_handles():
    ...
    global subplot1
       # model data
    distance = np.arange(min(PD_nm), max(PD_nm), 2)
